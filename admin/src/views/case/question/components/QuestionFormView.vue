    ...
    <ElForm :model="formModel" :rules="formRules" label-position="right" label-width="100px">
      <ElFormItem label="试题类型" prop="type">
        <ElSelect class="w-full" v-model="formModel.type" @change="onTypeChange">
          <ElOption v-for="item in useDictStore().items('question_type')" :key="item.value" :label="item.label"
            :value="item.value" />
        </ElSelect>
      </ElFormItem>
      ...
      <template v-if="formModel.type == '1' || formModel.type == '2'">
        <ElFormItem label="试题选项" prop="options">
          <div class="flex-1">
            ...
            <div class="pl-[30px]">
              <ElButton class="w-full" type="primary" plain :icon="Plus" @click="onAdd(0)"></ElButton>
            </div>
          </div>
        </ElFormItem>
      </template>
      ...
      <ElFormItem label="试题答案" prop="answer">
        ...
        <template v-if="formModel.type == '1'">
          <el-radio-group v-model="formModel.answer">
            <el-radio v-for="(item, index) in formModel.options" :key="item" :label="index">{{
              formatCapital(index)
            }}</el-radio>
          </el-radio-group>
        </template>
        ...
        <template v-else-if="formModel.type == '2'">
          <el-checkbox-group v-model="formModel.answer">
            <el-checkbox v-for="(item, index) in formModel.options" :key="item" :label="index">{{ formatCapital(index) }}
            </el-checkbox>
          </el-checkbox-group>
        </template>
...
const formRules: FormRules = {
  title: [{ required: true, message: '请输入试题内容', trigger: 'blur' }],
  options: [{ required: true, message: '请输入试题选项', trigger: 'blur' }],
