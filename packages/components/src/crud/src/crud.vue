  ...
  <section class="d-crud">
    ...
    <section v-loading="_currentTableLoading">
      ...
      <section class="d-crud-action">
        <el-button
          v-if="actionBtnShowHandle(__actionAttrs.isCreateBtn, _currentModelValue)"
          v-authorize="__actionAttrs.createBtnPermission"
          type="primary"
          :icon="CirclePlus"
          @click="onOpenCreate"
        >
      ...
      <el-table ref="tableRef" :data="_currentTableData" v-bind="{ ...$attrs, ...__tableAttrs }">
        ...
        <!-- 选择 selectionColumn -->
        <el-table-column
          v-if="__selectionColumn.isShow === true"
          v-bind="__selectionColumn.attrs"
          type="selection"
        />

        <!-- 序号 indexColumn -->
        <el-table-column
          v-if="__indexColumn.isShow === true"
          v-bind="__indexColumn.columnAttrs"
          type="index"
        />

        <!-- 动态列 -->
        <el-table-column
          v-for="field in tableFilterFields"
          :key="field.prop"
          :prop="field.prop"
          :label="field.label"
          ...
          <template #default="scopeProps">
            <slot :name="field.prop" v-bind="scopeProps" :field="field">
              <template v-if="field.type == 'image' || field.type == 'images'">
                <ImageList
                  :images="formatValue(scopeProps.row, field)"
                  width="40px"
                  height="40px"
                ></ImageList>
              </template>
        ...
        <el-table-column v-if="__actionAttrs.isRowAction" v-bind="__actionAttrs.rowActionAttrs">
          <template #default="scopeProps">
            <div class="d-crud-column-action">
              <el-button
                v-if="actionBtnShowHandle(__actionAttrs.isInfoBtn, scopeProps.row)"
                v-authorize="__actionAttrs.infoBtnPermission"
                :disabled="actionDisabledHandle(__actionAttrs.infoBtnDisabled, scopeProps.row)"
                text
                type="info"
                size="small"
                :icon="View"
                @click="onOpenInfo(scopeProps.row)"
              >
                详情
              </el-button>
              <el-button
                v-if="actionBtnShowHandle(__actionAttrs.isUpdateBtn, scopeProps.row)"
                v-authorize="__actionAttrs.updateBtnPermission"
                :disabled="actionDisabledHandle(__actionAttrs.updateBtnDisabled, scopeProps.row)"
                text
                type="primary"
                size="small"
                :icon="Edit"
                @click="onOpenUpdate(scopeProps.row)"
              >
                修改
              </el-button>
              <el-button
                v-if="actionBtnShowHandle(__actionAttrs.isDeleteBtn, scopeProps.row)"
                v-authorize="__actionAttrs.deleteBtnPermission"
                :disabled="actionDisabledHandle(__actionAttrs.deleteBtnDisabled, scopeProps.row)"
                text
                type="danger"
                size="small"
                :icon="Delete"
                @click="onDelete(scopeProps.row)"
              >
                删除
              </el-button>
    ...
    <!-- 弹窗区域 -->
    <el-dialog
      :class="{ 'd-dialog': true, 'd-dialog__center': __dialogAttrs.alignCenter }"
      v-model="dialogVisible"
      :close-on-click-modal="dialogTypeMap[currentStatus]?.closeOnClickModal"
    ...
    >
      <template #header>
        <div> {{ __dialogAttrs.title ?? dialogTypeMap[currentStatus]?.title }} </div>
        <button
      ...
      <template #default>
        <el-scrollbar :max-height="isFullscreen ? undefined : __dialogAttrs.height">
          <div class="d-dialog-wrapper">
            <DForm
              ref="formRef"
              v-if="currentStatus === 'create' || currentStatus === 'update'"
              :type="currentStatus"
              :option="option"
